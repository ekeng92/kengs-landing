...
curl -s http://localhost:8787/tasks?workspace_id=<WS_ID>&status=todo | jq '.data[] | {ref_code, title, priority}'
...
```bash
curl -s -X PATCH http://localhost:8787/tasks/AEON-NNN/move -H "Content-Type: application/json" -d '{"status": "in_progress"}'
```

**Complete a task** — move it to `review` with notes on what was done:
```bash
curl -s -X PATCH http://localhost:8787/tasks/AEON-NNN -H "Content-Type: application/json" \
...
```bash
curl -s -X POST http://localhost:8787/tasks -H "Content-Type: application/json" \
